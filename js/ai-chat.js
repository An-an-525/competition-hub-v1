...
var statusEl=document.getElementById('aiLoginStatus');if(statusEl){statusEl.innerHTML='<span style="color:var(--accent)">● 在线模式</span> · MiniMax M2.7';statusEl.dataset.mode='online'}// 初始化深度模式开关状态
...
var _fu=getCurrentUser();var _fuid=_fu?_fu.id:'guest';var messages=getLS('ai_messages_'+_fuid,[]);messages.push({role:'assistant',content:finalContent,time:new Date().toISOString(),thinking:thinkingText||undefined});if(messages.length>200)messages=messages.slice(-200);setLS('ai_messages_'+_fuid,messages)}}
    ...
    while (true) {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        try {
          ...
          // 检查内容审核
          if (data.input_sensitive) {
            onError('input_sensitive');
            return;
          }
          if (data.output_sensitive) {
            onError('output_sensitive');
            return;
          }
          ...
          if (delta.reasoning_content && deepMode) {
          ...
          // 非 JSON 数据，跳过
          console.warn('[SSE] 解析数据失败:', e.message);
        }
      }
    }
